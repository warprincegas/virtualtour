...
bars.addEventListener("click", () => {
  if (spanElem.innerHTML === "Menu") {
    ...
    document
      .querySelectorAll(
        ".pop-home, .pop-about, .pop-services, .pop-more, .pop-visit"
      )
      .forEach((elem) => {
    ...
    document.querySelector("nav").classList.add("z-index");
  } else {
    headerDisplay();
  }
});
...
document
  .querySelectorAll(".body-section, .body-section-pop, .left-section, footer")
  .forEach((elem) => {
    elem.addEventListener("click", () => {
      ...
      document
        .querySelectorAll(".home, .about, .services, .more, .visit")
        .forEach((elem) => {
          elem.classList.remove("translate");
        });
      document.querySelector("nav").classList.remove("z-index");
    });
  });
...
document.querySelector(".side-bars-options").addEventListener("click", () => {
  document.querySelector(".side-bar").classList.remove("visiblity");
...
const img = document.querySelectorAll(".sliding-images img");
